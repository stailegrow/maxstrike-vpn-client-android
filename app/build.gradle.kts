...
android {
    ...
    defaultConfig {
        ...
        // 65-75 МБ, и без фильтра итоговый APK весит ~280 МБ, хотя реальному
        // телефону нужна ровно одна архитектура. arm64-v8a покрывает
        // подавляющее большинство Android-устройств с 2017 года (в т.ч.
        // Samsung S25) — оставляем только её, отдаём один компактный APK
        // (~70 МБ) вместо "универсального", но неудобного для скачивания.
        // x86/x86_64 (эмуляторы) и armeabi-v7a (старые 32-битные телефоны)
        // сознательно не собираются — если понадобится собрать под них,
        // просто расширить список ниже.
        ndk {
